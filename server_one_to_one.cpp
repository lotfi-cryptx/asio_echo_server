...
public:
    ...
    static std::shared_ptr<Client> create(tcp::socket sock)
    {
        return std::shared_ptr<Client>(new Client(std::move(sock)));
    }
    ...
    void start()
    {
        ...
        std::thread([self, this]()
                    {
                        cout << "Thread started for " << this->_ep << endl;
                        this->_ioc.run();
                    }
                    ).detach();
    }
...
private:
    Client(tcp::socket sock):
        _ioc(),
        _sock(_ioc),
        _ep(sock.remote_endpoint())
    {
        auto fd = sock.release();
        _sock.assign(tcp::v4(), fd);
...
public:
    awaitable<void> _listen()
    {
        ...
        for (;;)
        {
            ...
            Client::create(std::move(client_sock))->start();
        }
    }
